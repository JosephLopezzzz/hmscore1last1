-- Database integration for new features (FIXED VERSION)
...
-- Insert sample room data (only if rooms table is empty or missing data)
-- First check if we have any rooms data
SET @room_count = (SELECT COUNT(*) FROM rooms);

-- If no rooms exist, insert sample data
-- Note: We'll use INSERT IGNORE to avoid duplicates
...
-- Update existing reservations with contact numbers and special requests (if they don't have them)
UPDATE reservations SET 
    contact_number = '+63 912 345 6789',
    special_requests = 'Late check-in requested'
WHERE guest_name = 'John Doe' AND (contact_number IS NULL OR contact_number = '');

UPDATE reservations SET 
    contact_number = '+63 917 123 4567',
    special_requests = 'High floor preferred'
WHERE guest_name = 'Mary Smith' AND (contact_number IS NULL OR contact_number = '');
